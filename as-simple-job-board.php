<?php
/**
 * Plugin Name:  AS Simple Job Board
 ...
 * Version: 2.1
 * Author: Anurag Singh
 * Author URI: https://github.com/anurag-singh
 * Text Domain:
 * License: GPL2
 * Plugin Type: Piklist
*/
...
// add new role 'human_resource' on plugin activation
register_activation_hook( __FILE__, 'add_user_role_hr' );
function add_user_role_hr() {
   add_role(
                'human_resource',
                __( 'Human Resource' ),
                array(
...
// remove role 'human_resource' on plugin deactivation
register_deactivation_hook(__FILE__, 'remove_user_role_hr');
function remove_user_role_hr() {
    remove_role('human_resource');
}
...
/* Add roles to administer the custom post types */
require_once 'includes/simple-job-board-add-user-roles-for-cpt.php';
...
require_once 'as-simple-job-board-template.php';
